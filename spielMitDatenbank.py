import sqlite3
con = sqlite3.connect("dame.db")
cur = con.cursor()
# sql = "DROP TABLE Score"
# sql = "CREATE TABLE Scoreboard(Username TEXT, Score INTEGER)"
# cur.execute(sql)
# sql = "DELETE FROM Score"
# cur.execute(sql)
# sqlSetScore = "INSERT INTO Score(Username, Game, Score, Date) VALUES('AAAA','Dame', 0, datetime())"
# cur.execute(sqlSetScore)
# con.commit()


loginFunction = input('Please login or continue as guest. Login, Guest ')
if loginFunction == 'Login':
    username = input('Username: ')
    password = input('Password: ')

    sqlCheckUser = "SELECT * FROM Users"
    res = cur.execute(sqlCheckUser)
    if res.fetchall() is not None:
        
        sqlCheckUser = "SELECT Username, Password FROM Users WHERE Username = '" + username + "'"
        res = cur.execute(sqlCheckUser)
        usernameCheck = res.fetchall()
        
        if any(username in sub_list for sub_list in usernameCheck)  and  any(password in sub_list for sub_list in usernameCheck):
            print("Logged in")
        elif any(username in sub_list for sub_list in usernameCheck) and any(password not in sub_list for sub_list in usernameCheck) :
    ...
    print('Your scores will not be saved!')

choise = input('What do you want to do? Play, Score ')

if choise != 'Play':
    game = input("What game? Tic-Tac-Toe, Dame ")
    sqlDeleteBoard = "DELETE FROM Scoreboard"
    cur.execute(sqlDeleteBoard)
    sqlScore = "SELECT Username FROM Score WHERE Game = '"+game+"' GROUP BY Username "
    res = cur.execute(sqlScore)
    users = res.fetchall()
    for i in users:
        user = str(i).strip(',)').strip('(')
        win = ""
        los = ""
        all = ""
        sqlScore = "SELECT Count(Score) FROM Score WHERE Score = 1 and Username = "+user+" GROUP BY Username"
        resWin = cur.execute(sqlScore)
        for i in resWin:
            win = str(i).strip(',)').strip('(')
        sqlScore = "SELECT Count(Score) FROM Score WHERE Score = 0 and Username = "+user+" GROUP BY Username "
        resLos = cur.execute(sqlScore)
        for i in resLos:
            los = str(i).strip(',)').strip('(')
        sqlScore = "SELECT Count(Score) FROM Score WHERE Username = "+user+" GROUP BY Username "
        resAll = cur.execute(sqlScore)
        for i in resAll:
            all = str(i).strip(',)').strip('(')
        result =(100 / int(all)) * int(win)
        str(result)
        sqlPercent = "INSERT INTO Scoreboard(Username, Score) Values("+user+",'" 
        sqlPercent = sqlPercent + str(result) + "')"
        res = cur.execute(sqlPercent)
        con.commit()
    sqlShowBoard = "SELECT Username FROM Scoreboard ORDER BY Score DESC"
    res = cur.execute(sqlShowBoard)
    a = 1
        ...
        def checkWinner(player):
            ...
            if row1_1 or row1_2 or row1_3 or row2_1 or row2_2 or row2_3 or row3_1 or row3_2 or row3_3 or row4_1 or row4_2 or row4_3 or row5_1 or row5_2 or row5_3 or row6_1 or row6_2 or row6_3 or column1_1 or column1_2 or column1_3 or column1_1 or column1_2 or column1_3 or column2_1 or column2_2 or column2_3 or column3_1 or column3_2 or column3_3 or column4_1 or column4_2 or column4_3 or column5_1 or column5_2 or column5_3 or column6_1 or column6_2 or column6_3 or diagonal1 or diagonal2_1 or diagonal2_2 or diagonal3_1 or diagonal3_2 or diagonal3_3 or diagonal4_1 or diagonal4_2 or diagonal5 or diagonal6 or diagonal7_1 or diagonal7_2 or diagonal8_1 or diagonal8_2 or diagonal8_3 or diagonal9_1 or diagonal9_2 or diagonal10:
                if player == "X":
                    ...
                    if loginFunction == 'Login':
                        sqlSetScore = "INSERT INTO Score(Username, Game, Score, Date) VALUES('"+username+"','Tic-Tac-Toe', 1, datetime())"
                        cur.execute(sqlSetScore)
                        con.commit()
                else:
                    print(player + " is the winner!")
                    if loginFunction == 'Login':
                        sqlSetScore = "INSERT INTO Score(Username, Game, Score, Date) VALUES('"+username+"','Tic-Tac-Toe', 0, datetime())"
                        cur.execute(sqlSetScore)
                        con.commit()
                return True
            return False
    ...
    if game == 'Dame':
       class Game:
            def __init__(self):
                ...
                # sqlSetScore = "INSERT INTO Score(Username, Game, Score, Date) VALUES('"+username+"','Dame', 1, datetime())"
                # cur.execute(sqlSetScore)
                # con.commit()
                ...
                while not self.game_over:
                    ...
                    if self.make_move(start_row, start_col, end_row, end_col):
                        if self.is_game_over():
                            if self.current_player == 'X':
                                print("Congratulations! Player you wins!")
                                if loginFunction == 'Login':
                                    sqlSetScore = "INSERT INTO Score(Username, Game, Score, Date) VALUES('"+username+"','Dame', 1, datetime())"
                                    cur.execute(sqlSetScore)
                                    con.commit()
                                ...
                                if loginFunction == 'Login':
                                    sqlSetScore = "INSERT INTO Score(Username, Game, Score, Date) VALUES('"+username+"','Dame', 0, datetime())"
                                    cur.execute(sqlSetScore)
                                    con.commit()
                    else:
