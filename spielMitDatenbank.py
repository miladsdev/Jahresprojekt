...
cur = con.cursor()
# cur.execute("CREATE TABLE Users(UserID INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, Username TEXT NOT NULL UNIQUE, Password TEXT )")
# cur.execute("CREATE TABLE Score(Username TEXT NOT NULL, Game TEXT NOT NULL, Score INTEGER )")
...
# print('Please login or continue as guest')
...
if loginFunction == 'Login':
    ...
    if res.fetchall() is not None:
        sqlCheckUser = "SELECT Username FROM Users WHERE Username = '" + username + "'"
        sqlCheckPassword = "SELECT Password FROM Users WHERE Password = '" + password + "'"
        test = cur.execute(sqlCheckPassword)
        passwordCheck = test.fetchall()
        res = cur.execute(sqlCheckUser)
        usernameCheck = res.fetchall()
        
        if any(username in sub_list for sub_list in usernameCheck) and  any(password in sub_list for sub_list in passwordCheck):
            print("Logged in")
        else:
...
if choise == 'Play':
    ...
    print('play')
    sqlGame = "INSERT INTO Users(Username, Password) Values('" + username + "' , '" + password + "')"
    cur.execute(sqlGame)
#     if game == 'Dame':
#         class Game:
#             def __init__(self):
...
#                     start_row = int(
#                         input("Enter the row of the piece you want to move: "))
#                     start_col = int(
#                         input("Enter the column of the piece you want to move: "))
#                     end_row = int(
#                         input("Enter the row where you want to move the piece: "))
#                     end_col = int(
#                         input("Enter the column where you want to move the piece: "))
